...
//   const percentageA = parseFloat(a["% Partido"].replace("%", ""))
//   const percentageB = parseFloat(b["% Partido"].replace("%", ""))
          ...
          .map((item) => (
            ...
            >
              <div className='flex w-full h-3 max-w-sm overflow-hidden bg-gray-300 '>
                {/* <div className='flex flex-col justify-center text-center text-white bg-blue-500 text-xxs ' role='progressbar' style={{ backgroundColor: item["Color"], width: `${parseFloat(item["Porcentaje Partido"].replace("%", ""))}%` }} aria-valuemin={0} aria-valuemax={100}>
                  {item["% Partido"]}
                </div> */}
              </div>
              <div className='flex flex-col p-1 rounded-b-lg'>
                ...
                <div className='flex flex-col h-full p-1 bg-white rounded-b-lg flex-2'>
                  <h2 className='text-2xl font-bold text-center text-gray-800 '>{item["% Partido"]}</h2>
                  ...
                  {/* <p className='inline-block m-2 text-center text-gray-500 text-xxs align-end'>Votos: {item["Votos Totales"]}</p> */}
                </div>
              </div>
            </div>
          ))}
